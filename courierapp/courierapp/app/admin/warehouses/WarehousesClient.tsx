...
interface WarehouseRow {
  id: string; country_id: string; name: string
  address_line1: string; city: string; zip: string
  phone: string; email: string | null
  is_origin: boolean; is_active: boolean
  countries?: { name_ru: string; code: string }
}

const emptyForm = {
  country_id: '', name: '', address_line1: '', address_line2: '',
  city: '', state: '', zip: '', phone: '', email: '',
  is_origin: true, is_active: true,
}

export default function WarehousesClient({ warehouses: initial, countries }: {
  warehouses: WarehouseRow[]; countries: Country[]
}) {
  ...
  // Countries sorted by order, only those that have warehouses
  const availableCountries = useMemo(() => {
    const codes = new Set(warehouses.map(w => w.countries?.code).filter(Boolean))
  ...
  const filtered = useMemo(() =>
    warehouses.filter(w => w.countries?.code === filterCountry),
  ...
  function openEdit(w: WarehouseRow) {
    setEditing(w)
    setForm({ country_id: w.country_id, name: w.name, address_line1: w.address_line1, address_line2: '', city: w.city, state: '', zip: w.zip, phone: w.phone, email: w.email ?? '', is_origin: w.is_origin, is_active: w.is_active })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = { country_id: form.country_id, name: form.name, address_line1: form.address_line1, address_line2: form.address_line2 || null, city: form.city, state: form.state || null, zip: form.zip, phone: form.phone, email: form.email || null, is_origin: form.is_origin, is_active: form.is_active }
    if (editing) {
    ...
    const { data } = await supabase.from('warehouses').select('*, countries(name_ru, code)').order('created_at', { ascending: false })
    setWarehouses(data ?? []); setSuccess(true); setLoading(false)
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openCreate} className="btn-primary"><Plus size={16} /> Добавить склад</button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          <button onClick={openCreate} className="btn-primary mt-4 mx-auto"><Plus size={16} /> Добавить склад</button>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map(w => (
            <div key={w.id} className="card p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 font-mono">{w.countries?.code}</span>
                    {w.is_origin && <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">Отправка</span>}
                    {!w.is_active && <span className="text-xs px-2 py-0.5 rounded-full bg-red-50 text-red-600">Неактивен</span>}
                  </div>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => openEdit(w)} className="btn-ghost p-2"><Pencil size={15} /></button>
                <button onClick={() => handleDelete(w.id)} className="btn-ghost p-2 hover:text-red-600 hover:bg-red-50"><Trash2 size={15} /></button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-card-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input name="name" type="text" className="input" placeholder="ASER CARGO EXPRESS — USA" value={form.name} onChange={handleChange} required />
              </div>
              <div>
                <label className="label">Адрес (строка 1)</label>
                <input name="address_line1" type="text" className="input" placeholder="123 Main St" value={form.address_line1} onChange={handleChange} required />
              </div>
              <div>
                <label className="label">Адрес (строка 2)</label>
                <input name="address_line2" type="text" className="input" placeholder="Suite 100" value={form.address_line2} onChange={handleChange} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="label">Город</label><input name="city" type="text" className="input" placeholder="New York" value={form.city} onChange={handleChange} required /></div>
                <div><label className="label">ZIP / Индекс</label><input name="zip" type="text" className="input" placeholder="10001" value={form.zip} onChange={handleChange} required /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="label">Телефон</label><input name="phone" type="text" className="input" placeholder="+1 212 000 0000" value={form.phone} onChange={handleChange} required /></div>
                <div><label className="label">Email</label><input name="email" type="email" className="input" placeholder="warehouse@example.com" value={form.email} onChange={handleChange} /></div>
              </div>
              <div className="flex items-center gap-6 pt-1">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" name="is_origin" checked={form.is_origin} onChange={handleChange} className="w-4 h-4 rounded accent-blue-600" />
